...
public abstract class SWActor extends Actor<SWActionInterface> implements SWEntityInterface {
	...
	/**
	 * The amount of <code>hitpoints</code> of this actor. If the hitpoints are
	 * zero or less this <code>Actor</code> is dead
	 */
	...
	/**
	 * If or not this <code>SWActor</code> is human controlled.
	 * <code>SWActor</code>s are not human controlled by default
	 */
	protected boolean humanControlled = false;

	/**
	 * A string symbol that represents this <code>SWActor</code>, suitable for
	 * display
	 */
	 ...
	 * 
	 * @param team
	 *            to which this <code>SWActor</code> belongs to
	 * @param hitpoints
	 *            initial hitpoints of this <code>SWActor</code> to start with
	 * @param m
	 *            message renderer for this <code>SWActor</code> to display
	 *            messages
	 * @param world
	 *            the <code>World</code> to which <code>SWActor</code> belongs
	 *            to
	 * 
	 * @see #team
	 * @see #hitpoints
	 ...
	 * 
	 * @param s
	 *            the new <code>Scheduler</code> of this <code>SWActor</code>
	 * @see #scheduler
	 */
	...
	public ArrayList<SWActionInterface> getActions() {
		...
		// If the HobbitActor is carrying anything, look for its affordances and
	 ...
	 * <p>
	 * This method only returns the reference of the item carried and does not
	 * remove the item held from this <code>SWActor</code>.
	 * <p>
	 * If this <code>SWActor</code> is not carrying an item this method will
	 * return null.
	 * 
	 * @return the item carried by this <code>SWActor</code> or null if no item
	 *         is held by this <code>SWActor</code>
	 * @see #itemCarried
	 */
	...
	/**
	 * Returns the actor's Force object.
	 * 
	 * @return SWActor's instance of Force.
	 */
	 ...
	 * <code>team</code>.
	 * <p>
	 * Useful when the <code>SWActor</code>'s team needs to change dynamically
	 * during the simulation. For example, a bite from an evil actor makes a
	 * good actor bad.
	 *
	 * @param team
	 *            the new team of this <code>SWActor</code>
	 * @see #team
	 */
	...
	/**
	 * Method insists damage on this <code>SWActor</code> by reducing a certain
	 * amount of <code>damage</code> from this <code>SWActor</code>'s
	 * <code>hitpoints</code>
	 * 
	 * @param damage
	 *            the amount of <code>hitpoints</code> to be reduced
	 * @pre <code>damage</code> should not be negative
	 */
	@Override
	public void takeDamage(int damage) {
		// Precondition 1: Ensure the damage is not negative. Negative damage
		// could increase the SWActor's hitpoints
		assert (damage >= 0) : "damage on SWActor must not be negative";
		this.hitpoints -= damage;
	}

	/**
	 * Assigns this <code>SWActor</code>'s <code>itemCarried</code> to a new
	 * item <code>target</code>
	 * <p>
	 * This method will replace items already held by the <code>SWActor</code>
	 * with the <code>target</code>. A null <code>target</code> would signify
	 * that this <code>SWActor</code> is not carrying an item anymore.
	 * 
	 * @param target
	 *            the new item to be set as item carried
	 * @see #itemCarried
	 */
	 ...
	 * <p>
	 * A <code>SWActor</code> is dead when it's <code>hitpoints</code> are less
	 * than or equal to zero (0)
	 *
	 * @author ram
	 * @return true if and only if this <code>SWActor</code> is dead, false
	 *         otherwise
	 * @see #hitpoints
	 */
	 ...
	 * <p>
	 * Human controlled <code>SWActors</code>' <code>SWActions</code> are
	 * selected by the user as commands from the Views.
	 * 
	 * @return true if the <code>SWActor</code> is controlled by a human, false
	 *         otherwise
	 * @see #humanControlled
	 */
	...
	/**
	 * This method will poll this <code>SWActor</code>'s current
	 * <code>Location loc</code> to find potential exits, and replaces all the
	 * instances of <code>Move</code> in this <code>SWActor</code>'s command set
	 * with <code>Moves</code> to the new exits.
	 * <p>
	 ...
	 * @author ram
	 * @param loc
	 *            this <code>SWActor</code>'s location
	...
	public void resetMoveCommands(Location loc) {
		...
		for (CompassBearing d : CompassBearing.values()) {
			if (loc.getNeighbour(d) != null) // if there is an exit from the
												// current location in direction
												// d, add that as a Move command
		...
		// TODO: This assumes that the only actions are the Move actions. This
		// will clobber any others. Needs to be fixed.
		/*
