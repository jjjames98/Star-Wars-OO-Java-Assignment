...
/**
 * A very minimal <code>SWActor</code> that the user can control. Its
...
public class Player extends SWActor {
	 ...
	 * <code>SWActor</code></li>
	 * </ul>
	 * 
	 * @param team
	 *            the <code>Team</code> to which the this <code>Player</code>
	 *            belongs to
	 * @param hitpoints
	 *            the hit points of this <code>Player</code> to get started with
	 * @param m
	...
	public Player(Team team, int hitpoints, MessageRenderer m, SWWorld world) {
		...
		humanControlled = true; // this feels like a hack. Surely this should be
								// dynamic
		force.setCanUse(true);
		force.setForceAbility(2);
	}
	 ...
	 * <p>
	 * This method will only be called if this <code>Player</code> is alive and
	 * is not waiting.
	 * 
	...
	public void describeScene() {
		...
		if (itemCarried != null) {
			// and describe the item carried if the player is actually carrying
			// an item
			say(this.getShortDescription() + " is holding " + itemCarried.getShortDescription() + " ["
					+ itemCarried.getHitpoints() + "]");
		}
		...
		if (contents.size() > 1) { // if it is equal to one, the only thing here
									// is this Player, so there is nothing to
									// report
